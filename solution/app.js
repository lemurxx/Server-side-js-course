...
app.use(morgan('dev'));
const dataservice = require('./services/db-service');
...
app.delete('/people/:id', (req, resp) => {
    dataservice.deletePerson(req.params.id, (err, data) => {
        if (err) {
            console.log(err);
            resp.status(500);
        } else {
...
app.put('/people/:id', (req, resp) => {
    dataservice.updatePerson(req.params.id, (err, data) => {
        if (err) {
            resp.status(500);
        } else {
            resp.json(data);
...
app.post('/planet', (req, resp) => {
    dataservice.insertPlanet(req.body, (err, data) => {
        if (err) {
            resp.status(500);
        } else {
            resp.json(data);
